...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	else {$fn = mysqli_real_escape_string($_POST['first_name']);}
	if (empty($_POST['last_name'])) {$errors[] = 'enter your last name.';}
	else {$ln = mysqli_real_escape_string($_POST['last_name']);}
	if (empty($_POST['email'])) {$errors[] = 'enter your email.';}
	else {$e = mysqli_real_escape_string($_POST['email']);}
	if (!empty($_POST['pass1'])) 
	{   
	if ($_POST['pass1'] != $_POST['pass2']) {$errors[] = 'Password did not match.';}
	else {$p = mysqli_real_escape_string($_POST['pass1']);}
	}
